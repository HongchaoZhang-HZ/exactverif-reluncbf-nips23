    ...
    for cell in data[sect_idx]:
        ...
        for num_layer in range(num_act_layer):
            ...
            if num_layer == 0:
                ...
                act_set_layers = np.vstack([act_set_layers, (act_set + 32 * num_layer)])
...
def verification(nnmodel, actual_set):
    ...
    for act_array in actual_set:
        W_overl, r_overl, B_act, B_inact = activated_weight_bias_ml(nnmodel, torch.Tensor(act_array), 32)
...
def main():
    ...
    nnmodel.load_state_dict(torch.load('obs_2_32.pt'), strict=True)

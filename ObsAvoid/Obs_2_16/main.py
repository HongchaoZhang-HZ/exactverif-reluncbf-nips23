    ...
    for cell in data[sect_idx]:
        ...
        for num_layer in range(num_act_layer):
            ...
            if num_layer == 0:
                ...
                act_set_layers = np.vstack([act_set_layers, (act_set + 16 * num_layer)])
...
def verification(nnmodel, actual_set):
    ...
    for act_array in actual_set:
        W_overl, r_overl, B_act, B_inact = activated_weight_bias_ml(nnmodel, torch.Tensor(act_array), 16)
...
def suf_nec_inter_verification(W_overl_inter, r_overl_inter, W_Bound_inter, r_Bound_inter, size):
    ...
    lcon = LinearConstraint(np.hstack([np.vstack([W_Bound_inter, np.zeros([2 * sum_range, W_Bound_inter.shape[1]])]),
                                       np.vstack([np.zeros([W_Bound_inter.shape[0], 2 * sum_range]), np.eye(2 * sum_range)])]),
    ...
    # np.vstack([W_overl_inter, np.zeros([2 * sum_range, W_overl_inter.shape[1]])])
    # np.vstack([np.zeros([W_overl_inter.shape[0], 2 * sum_range]), np.eye(2 * sum_range)])
    # eqcon = LinearConstraint(np.hstack([np.vstack([W_overl_inter, np.zeros([2 * sum_range, W_overl_inter.shape[1]])]),
    #                                     np.vstack([np.zeros([W_overl_inter.shape[0], 2 * sum_range]),
    #                                                np.eye(2 * sum_range)])]),
    #                          np.hstack([-r_overl_inter[:, 0], np.zeros(2 * sum_range)]),
    #                          np.hstack([-r_overl_inter[:, 0], np.zeros(2 * sum_range)]))
    eqcon = LinearConstraint(np.hstack([np.vstack([W_overl_inter, np.zeros([2 * sum_range, W_overl_inter.shape[1]])]),
                                        np.vstack([results_array.reshape([sum_range, 1]) * np.ones([W_overl_inter.shape[0], 2 * sum_range]), np.eye(2 * sum_range)])]),
                             np.hstack([-r_overl_inter[:,0], np.zeros(2*sum_range)]),
                             np.hstack([-r_overl_inter[:,0], np.zeros(2*sum_range)]))
    res = minimize(y_dbdxf, xy0.reshape(len(xy0)), args=W_overl_inter, constraints=[lcon, eqcon], tol=1e-6)
    return res.fun
...
def correctness_verification(nnmodel, actual_set):
    ...
    for act_array in actual_set:
        ...
        lcon = LinearConstraint(W_Bound, -np.inf*np.ones(len(W_Bound)), -r_Bound.reshape(len(r_Bound)))
        eqcon = LinearConstraint(-W_overl[0], r_overl[0], r_overl[0])
        res = minimize(h_x, x0, constraints=[lcon, eqcon], tol=1e-6)
...
def main():
    ...
    nnmodel.load_state_dict(torch.load('obs_2_16.pt'), strict=True)
    ...
    if veri_res_set and veri_cor_set and all(veri_res_intersect):

...
def activated_weight_bias_ml(model,activated_set,num_neuron):
    ...
    # W_overl = torch.matmul(W_list[-1], W_l)  # compute \overline{W}(S)
    # r_overl = torch.matmul(W_list[-1], r_l) + r_list[-1]  # compute \overline{r}(S)
    W_overl = torch.matmul(W_list[-2], W_l)  # compute \overline{W}(S)
    r_overl = torch.matmul(W_list[-2], r_l) + r_list[-2].reshape([len(r_list[-2]),1]) # compute \overline{r}(S)
    W_overl = torch.matmul(W_list[-1], W_overl)  # compute \overline{W}(S)
    r_overl = torch.matmul(W_list[-1], r_overl) + r_list[-1]  # compute \overline{r}(S)

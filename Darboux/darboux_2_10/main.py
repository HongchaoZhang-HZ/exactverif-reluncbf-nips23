...
def verification(nnmodel, actual_set):
    ...
    for act_array in actual_set:
        W_overl, r_overl, B_act, B_inact = activated_weight_bias_ml(nnmodel, torch.Tensor(act_array), 10)
...
def main():
    ...
    nnmodel.load_state_dict(torch.load('darboux_2_10.pt'), strict=True)
